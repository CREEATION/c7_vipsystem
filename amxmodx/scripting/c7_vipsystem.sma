...
#define GUNMENU_TITLE "\d[\rLPC\d] \yVIP \wGunmenu"
#define GUNMENU_STITLE "\d[\rLPC\d] \yVIP \wGunmenu^n\yPrimary Weapon\r"
#define GUNMENU_PTITLE "\d[\rLPC\d] \yVIP \wGunmenu^n\ySecondary Weapon"

#define CHAT_PREFIX "^4[LPC Gunmenu]^3 "
#define HUD_PREFIX ""

#define MSG_SETUP_RECIEVED "%sYou got ^1%s ^4+ ^1%s^3 and full equipment"
#define MSG_SETUP_SPAWN "%sYou will spawn with ^1%s ^4+ ^1%s^3 and full equipment"
#define MSG_SETUP_SAVED "%sSay ^4/guns ^3to select a new setup"

#define MSG_MENU_RESET "%sGunmenu will show again next round"
#define MSG_MENU_DISABLED "%sGunmenu disabled. Say ^4/guns ^3to enable it again"

#define MSG_BUYTIME_PASSED "%s%d seconds have passed.^rGunmenu will show again next round"
#define MSG_BUYTIME_LEFT "%sGunmenu will close in %d seconds^ror when leaving the buyzone"

#define MSG_NO_VIP "%sYou need to be VIP to see the gunmenu (^1say ^4/wantvip^3)"
#define MSG_ROUND_NOTREACHED "%sYou need to wait ^1%d %s ^3to see the gunmenu"

#define MSG_AWP_ALL "[ AWP is restricted for all players ]"
#define MSG_AWP_VIP "[ AWP is only available for VIPs ]"
#define MSG_AUTO_ALL "[ Auto-Sniper is restricted for all players ]"
#define MSG_AUTO_VIP "[ Auto-Sniper is only available for VIPs ]"
...
enum _:MAINMENU_OPTION {
	O_EMPTY, O_NEW_SETUP, O_PREV_SETUP, O_SAVE_SETUP, O_DISABLE, O_CLOSE
}
...
new Array:g_WeaponSetups,
	Array:g_WeaponNames,
	Array:g_PrimaryWeapons,	Array:g_SecondaryWeapons

new g_Mainmenu, g_PrimaryMenu, g_SecondaryMenu

new g_MsgMoney, g_VIPBonusMoney, g_MsgMoneyHook,
	g_HudDmgMade, g_HudDmgRecieved, g_HudHealthUp

static Float:g_Buytime, Float:g_RoundStartTime, bool:g_InFreezetime

// cvar pointers
new g_VIP_Flag[27]
new lpc_vip_enable,
	lpc_vip_gunmenu_enable,
	lpc_vip_gunmenu_round,
	lpc_vip_gunmenu_grens,
	lpc_vip_flag,
	lpc_vip_money_kill,
	lpc_vip_money_hs,
	lpc_vip_hp_kill,
...
public plugin_init()
{
	register_plugin( "LPC VIP Privileges", PLUGIN_VERSION, "CREE7EN." )

	//**************************************************************************************
	// register cvars
	lpc_vip_enable = register_cvar( "lpc_vip_enable", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_flag = register_cvar( "lpc_vip_flag", "t", FCVAR_SPONLY | FCVAR_UNLOGGED )
	// money bonus
	lpc_vip_money_kill = register_cvar( "lpc_vip_money_kill", "200", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_money_hs = register_cvar( "lpc_vip_money_hs", "400", FCVAR_SPONLY | FCVAR_UNLOGGED )
	// hp bonus
	lpc_vip_hp_kill = register_cvar( "lpc_vip_hp_kill", "15", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_hp_hs = register_cvar( "lpc_vip_hp_hs", "30", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_hp_max = register_cvar( "lpc_vip_hp_max", "100", FCVAR_SPONLY | FCVAR_UNLOGGED )
	// sniper restriction
	lpc_vip_awp = register_cvar( "lpc_vip_awp", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_autosniper = register_cvar( "lpc_vip_autosniper", "0", FCVAR_SPONLY | FCVAR_UNLOGGED )
	// gunmenu cvars
	lpc_vip_gunmenu_enable = register_cvar( "lpc_vip_gunmenu_enable", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_gunmenu_round = register_cvar( "lpc_vip_gunmenu_round", "3", FCVAR_SPONLY | FCVAR_UNLOGGED )
	lpc_vip_gunmenu_grens = register_cvar( "lpc_vip_gunmenu_grens", "1 2 1", FCVAR_SPONLY | FCVAR_UNLOGGED )
	...
	//**************************************************************************************
	// set initial values
	get_pcvar_string( lpc_vip_flag, g_VIP_Flag, charsmax(g_VIP_Flag) )
...
public plugin_precache()
{
	...
	server_cmd( "exec %s/_LPC/lpc_vip.cfg", g_ConfigsDir )
...
{
	if ( !is_user_connected(p_userid) || !get_pcvar_num(lpc_vip_gunmenu_enable) || !is_user_vip(p_userid) )
		return HAM_HANDLED
	...
	if ( !round_is_buytime_over() && !g_InFreezetime && !g_PlayerRecievedSetup[p_userid] )
	{
		...
		client_print( p_userid, print_center, MSG_BUYTIME_PASSED, HUD_PREFIX, floatround( g_Buytime, floatround_floor ) )
...
{
	if ( !get_pcvar_num(lpc_vip_enable) )
	...
	// no valid victim
	if ( i_Victim < 1 || i_Victim > MAX_PLAYERS )
		return PLUGIN_HANDLED
	...
	if ( is_user_vip(i_Victim) )
	{
		if ( task_exists(i_Victim) )
	...
	if ( !is_user_connected(i_Attacker) )
		return PLUGIN_HANDLED

	// no valid attacker
	if ( i_Attacker < 1 || i_Attacker > MAX_PLAYERS )
		return PLUGIN_HANDLED
	...
	static n_BonusMoney, n_HealthBonus, n_HealthMax, n_HealthVIP, n_ArmorVIP, n_ArmorBonus

	// headshot bonus?
	if ( i_Headshot == 1 )
	{
		n_HealthBonus = get_pcvar_num(lpc_vip_hp_hs)
		n_BonusMoney = get_pcvar_num(lpc_vip_money_hs)
	}
	else
	{
		n_HealthBonus = get_pcvar_num(lpc_vip_hp_kill)
		n_BonusMoney = get_pcvar_num(lpc_vip_money_kill)
	}
	...
	// add health bonus
	n_HealthMax = get_pcvar_num(lpc_vip_hp_max)
	n_HealthVIP = get_user_health(i_Attacker)

	// show added hp, if not already 100 HP
	...
	if ( n_HealthVIP >= n_HealthMax )
	{
		...
		if ( i_Headshot == 1 )
			n_ArmorBonus = get_pcvar_num(lpc_vip_hp_hs) / 2
		else
			n_ArmorBonus = get_pcvar_num(lpc_vip_hp_kill) / 2
...
// https://forums.alliedmods.net/showthread.php?t=104887
...
{
	if ( !is_user_connected(p_victim) || !get_pcvar_num(lpc_vip_enable) )
		return PLUGIN_CONTINUE

	new n_Damage, i_Attacker
	...
	// show dmg to vips if not self-inflicted damage & victim is alive
	...
	for ( new i; i < n_Players; i++ )
	{
		...
		if ( p_victim != i_Attacker && i_Attacker <= MAX_PLAYERS )
			if ( is_spectating( a_Players[i], i_Attacker ) )
...
{
	if ( !is_user_connected(p_userid) || !get_pcvar_num(lpc_vip_enable) )
		return PLUGIN_CONTINUE
...
{
	if ( !is_user_connected(p_userid) || !is_user_alive(p_userid) || !get_pcvar_num(lpc_vip_gunmenu_enable) )
		return PLUGIN_CONTINUE
...
{
	if ( !get_pcvar_num(lpc_vip_gunmenu_enable) )
		return PLUGIN_CONTINUE
...
public event_round_new()
{
	...
	// read cvars again
	set_pcvar_num( lpc_vip_enable, get_pcvar_num(lpc_vip_enable) )
	get_pcvar_string( lpc_vip_flag, g_VIP_Flag, charsmax(g_VIP_Flag) )
	set_pcvar_string( lpc_vip_flag, g_VIP_Flag )
	set_pcvar_num( lpc_vip_money_kill, get_pcvar_num(lpc_vip_money_kill) )
	set_pcvar_num( lpc_vip_money_hs, get_pcvar_num(lpc_vip_money_hs) )
	set_pcvar_num( lpc_vip_hp_kill, get_pcvar_num(lpc_vip_hp_kill) )
	set_pcvar_num( lpc_vip_hp_hs, get_pcvar_num(lpc_vip_hp_hs) )
	set_pcvar_num( lpc_vip_hp_max, get_pcvar_num(lpc_vip_hp_max) )
	set_pcvar_num( lpc_vip_awp, get_pcvar_num(lpc_vip_awp) )
	set_pcvar_num( lpc_vip_autosniper, get_pcvar_num(lpc_vip_autosniper) )
	// gunmenu
	set_pcvar_num( lpc_vip_gunmenu_enable, get_pcvar_num(lpc_vip_gunmenu_enable) )
	set_pcvar_num( lpc_vip_gunmenu_round, get_pcvar_num(lpc_vip_gunmenu_round) )
...
{
	if ( !get_pcvar_num(lpc_vip_gunmenu_enable) || !is_gunmenu_enabled() )
		return PLUGIN_CONTINUE
	...
	for ( new i; i < n_Players; i++ )
	{
		if ( !is_user_vip(a_Players[i]) )
		{
			client_print_color( a_Players[i], Grey, MSG_NO_VIP, CHAT_PREFIX )
			continue
		}

		if ( g_PlayerSavedSetup[a_Players[i]] || g_PlayerSpawnSetup[a_Players[i]] || g_PlayerRecievedSetup[a_Players[i]] || g_PlayerDisableMenu[a_Players[i]] )
			continue
		...
		if ( !g_PlayerSavedSetup[a_Players[i]] && !g_PlayerSpawnSetup[a_Players[i]] && !g_PlayerDisableMenu[a_Players[i]] )
		{
...
populate_menus()
{
	...
	if ( !g_FilePointer )
	{
		set_fail_state( "[LPC] Gunmenu Configuration File not found" )
		return PLUGIN_HANDLED
	}

	new s_Line[64], s_WeaponName[32],
		n_SetupCount

	// create menus
	g_Mainmenu = menu_create( GUNMENU_TITLE, "handle_mainmenu" )
	g_PrimaryMenu = menu_create( GUNMENU_STITLE, "handle_primary_menu" )
	g_SecondaryMenu = menu_create( GUNMENU_PTITLE, "handle_secondary_menu" )
	...
	while ( !feof(g_FilePointer) )
	{
		...
		new s_WeaponConst[32], s_LineR[32]
...
populate_mainmenu()
{
	...
	// add a blank item (if any setups)
...
public handle_buy_menu( p_userid, p_key )
{
	...
	if ( ( u_Team == 1 && p_key == 4 ) || ( u_Team == 2 && p_key == 5 ) )
		handle_menu_weapon( p_userid, lpc_vip_awp )
	if ( ( u_Team == 1 && p_key == 5 ) || ( u_Team == 2 && p_key == 4 ) )
		handle_menu_weapon( p_userid, lpc_vip_autosniper )
...
public handle_menu_weapon( p_userid, p_pcvar )
{
	...
	{
		client_print( p_userid, print_center, MSG_AWP_ALL )
	...
	{
		client_print( p_userid, print_center, MSG_AWP_VIP )
		engclient_cmd( p_userid, "menuselect", "10" )
		return PLUGIN_HANDLED
	}
...
{
	handle_buy_weapon( p_userid, lpc_vip_awp )
}

public handle_buy_autosniper( p_userid )
{
	handle_buy_weapon( p_userid, lpc_vip_autosniper )
}
...
public event_current_weapon( p_userid )
{
	...
	if ( get_pcvar_num(lpc_vip_awp) == 2 || get_pcvar_num(lpc_vip_autosniper) == 2 )
		return PLUGIN_HANDLED

	switch ( get_user_weapon( p_userid, _, _ ) )
	{
		case CSW_AWP: handle_current_weapon( p_userid, lpc_vip_awp, "weapon_awp", MSG_AWP_ALL, MSG_AWP_VIP )
		case CSW_SG550: handle_current_weapon( p_userid, lpc_vip_autosniper, "weapon_sg550", MSG_AUTO_ALL, MSG_AUTO_VIP )
		case CSW_G3SG1: handle_current_weapon( p_userid, lpc_vip_autosniper, "weapon_g3sg1", MSG_AUTO_ALL, MSG_AUTO_VIP )
	}
...
{
	if ( !is_user_connected(p_userid) || !get_pcvar_num(lpc_vip_gunmenu_enable) )
		return PLUGIN_HANDLED

	if ( !is_user_vip(p_userid) )
	{
		client_print_color( p_userid, Grey, MSG_NO_VIP, CHAT_PREFIX )
		return PLUGIN_HANDLED
	}
	...
	else
	{
		if ( !is_user_alive(p_userid) )
		{
			...
			client_print( p_userid, print_center, MSG_MENU_RESET, HUD_PREFIX )
...
stock user_show_mainmenu( p_userid )
{
	...
	new i_PreviousSetup, i_PreviousSetupSave
	...
	{
		new s_PrimaryWeapon[32], s_SecondaryWeapon[32]
...
stock user_give_setup( p_userid, p_primary=-1, p_secondary=-1, bool:p_save=false )
{
	...
	{
		new s_SetupPrimaryC[15], s_SetupSecondaryC[17]
		...
		if ( p_save )
			client_print_color( p_userid, Grey, MSG_SETUP_SAVED, CHAT_PREFIX )
	}
	else
	{
		...
		client_print_color( p_userid, Grey, MSG_MENU_DISABLED, CHAT_PREFIX )
	}
...
stock user_give_equip( p_userid )
{
	...
	// get grenades amount
	new s_Grenades[9], n_HE[3], n_Flash[3], n_Smoke[3]
	get_pcvar_string( lpc_vip_gunmenu_grens, s_Grenades, charsmax(s_Grenades) )
...
// https://forums.alliedmods.net/showpost.php?p=2039952&postcount=4
...
{
	if ( !is_user_connected(p_userid) || !( get_user_flags(p_userid) & read_flags(g_VIP_Flag) ) )
		return false
...
public task_menu_timeleft( p_taskid )
{
	...
	if ( round_get_buytime_left() >= 0.1 && g_PlayerInBuyzone[i_User] )
		client_print( i_User, print_center, MSG_BUYTIME_LEFT, HUD_PREFIX, floatround( round_get_buytime_left(), floatround_ceil ) )
	else
		client_print( i_User, print_center, MSG_BUYTIME_PASSED, HUD_PREFIX, floatround( g_Buytime, floatround_floor ) )
}
...
{
	return g_CurrentRound >= get_pcvar_num(lpc_vip_gunmenu_round)
}

stock print_gunmenu_roundsleft( p_userid=0 )
{
	new s_RoundString[7] = "rounds",
		n_RoundsLeft = get_pcvar_num(lpc_vip_gunmenu_round) - g_CurrentRound

	if ( n_RoundsLeft <= 1 )
		formatex( s_RoundString, charsmax(s_RoundString), "round" )

	client_print_color(
		p_userid, Grey, MSG_ROUND_NOTREACHED, CHAT_PREFIX,
...
stock handle_menu_setup( p_userid, p_setupid )
{
	...
	new s_SetupConstants[18],
		s_SetupPrimaryC[15], s_SetupSecondaryC[17],
		i_Primary = -1, i_Secondary = -1
