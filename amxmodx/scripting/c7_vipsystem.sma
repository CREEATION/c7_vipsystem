...
new g_VIP_Flag[27]
new c7_vip_enable,
  c7_vip_gunmenu_enable,
  c7_vip_gunmenu_round,
  c7_vip_gunmenu_grens,
  c7_vip_flag,
  c7_vip_money_kill,
  c7_vip_money_hs,
  c7_vip_hp_kill,
...
public plugin_init()
{
  register_plugin( "C7 VIP Privileges", PLUGIN_VERSION, "CREE7EN." )

  // register dictionary
  register_dictionary("c7_vip.txt")

  // register cvars
  c7_vip_enable = register_cvar( "c7_vip_enable", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_flag = register_cvar( "c7_vip_flag", "t", FCVAR_SPONLY | FCVAR_UNLOGGED )

  // money bonus
  c7_vip_money_kill = register_cvar( "c7_vip_money_kill", "200", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_money_hs = register_cvar( "c7_vip_money_hs", "400", FCVAR_SPONLY | FCVAR_UNLOGGED )

  // hp bonus
  c7_vip_hp_kill = register_cvar( "c7_vip_hp_kill", "15", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_hp_hs = register_cvar( "c7_vip_hp_hs", "30", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_hp_max = register_cvar( "c7_vip_hp_max", "100", FCVAR_SPONLY | FCVAR_UNLOGGED )

  // sniper restriction
  c7_vip_awp = register_cvar( "c7_vip_awp", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_autosniper = register_cvar( "c7_vip_autosniper", "0", FCVAR_SPONLY | FCVAR_UNLOGGED )

  // gunmenu cvars
  c7_vip_gunmenu_enable = register_cvar( "c7_vip_gunmenu_enable", "1", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_gunmenu_round = register_cvar( "c7_vip_gunmenu_round", "3", FCVAR_SPONLY | FCVAR_UNLOGGED )
  c7_vip_gunmenu_grens = register_cvar( "c7_vip_gunmenu_grens", "1 2 1", FCVAR_SPONLY | FCVAR_UNLOGGED )
  ...
  // set initial values
  get_pcvar_string( c7_vip_flag, g_VIP_Flag, charsmax(g_VIP_Flag) )
...
public plugin_precache()
{
  ...
  server_cmd( "exec %s/c7_vip/plugin.cfg", g_ConfigsDir )
...
{
  if ( !is_user_connected(p_userid) || !get_pcvar_num(c7_vip_gunmenu_enable) || !is_user_vip(p_userid) )
    return HAM_HANDLED
...
{
  if ( !get_pcvar_num(c7_vip_enable) )
  ...
  if ( i_Headshot == 1 )
  {
    n_HealthBonus = get_pcvar_num(c7_vip_hp_hs)
    n_BonusMoney = get_pcvar_num(c7_vip_money_hs)
  }
  else
  {
    n_HealthBonus = get_pcvar_num(c7_vip_hp_kill)
    n_BonusMoney = get_pcvar_num(c7_vip_money_kill)
  }
  ...
  // add health bonus
  n_HealthMax = get_pcvar_num(c7_vip_hp_max)
  ...
  if ( n_HealthVIP >= n_HealthMax )
  {
    ...
    if ( i_Headshot == 1 )
      n_ArmorBonus = get_pcvar_num(c7_vip_hp_hs) / 2
    else
      n_ArmorBonus = get_pcvar_num(c7_vip_hp_kill) / 2
...
{
  if ( !is_user_connected(p_victim) || !get_pcvar_num(c7_vip_enable) )
    return PLUGIN_CONTINUE
...
{
  if ( !is_user_connected(p_userid) || !get_pcvar_num(c7_vip_enable) )
    return PLUGIN_CONTINUE
...
{
  if ( !is_user_connected(p_userid) || !is_user_alive(p_userid) || !get_pcvar_num(c7_vip_gunmenu_enable) )
    return PLUGIN_CONTINUE
...
{
  if ( !get_pcvar_num(c7_vip_gunmenu_enable) )
    return PLUGIN_CONTINUE
...
public event_round_new()
{
  ...
  // read cvars again
  set_pcvar_num( c7_vip_enable, get_pcvar_num(c7_vip_enable) )
  get_pcvar_string( c7_vip_flag, g_VIP_Flag, charsmax(g_VIP_Flag) )
  set_pcvar_string( c7_vip_flag, g_VIP_Flag )
  set_pcvar_num( c7_vip_money_kill, get_pcvar_num(c7_vip_money_kill) )
  set_pcvar_num( c7_vip_money_hs, get_pcvar_num(c7_vip_money_hs) )
  set_pcvar_num( c7_vip_hp_kill, get_pcvar_num(c7_vip_hp_kill) )
  set_pcvar_num( c7_vip_hp_hs, get_pcvar_num(c7_vip_hp_hs) )
  set_pcvar_num( c7_vip_hp_max, get_pcvar_num(c7_vip_hp_max) )
  set_pcvar_num( c7_vip_awp, get_pcvar_num(c7_vip_awp) )
  set_pcvar_num( c7_vip_autosniper, get_pcvar_num(c7_vip_autosniper) )
  // gunmenu
  set_pcvar_num( c7_vip_gunmenu_enable, get_pcvar_num(c7_vip_gunmenu_enable) )
  set_pcvar_num( c7_vip_gunmenu_round, get_pcvar_num(c7_vip_gunmenu_round) )
...
{
  if ( !get_pcvar_num(c7_vip_gunmenu_enable) || !is_gunmenu_enabled() )
    return PLUGIN_CONTINUE
...
populate_menus()
{
  ...
  get_configsdir( g_ConfigsDir, charsmax(g_ConfigsDir) )
  format( g_ConfigsDir, charsmax(g_ConfigsDir), "%s/c7_vip/gunmenu.ini", g_ConfigsDir )
...
{
  if ( !is_user_connected(p_userid) || !is_user_vip(p_userid) || !get_pcvar_num(c7_vip_gunmenu_enable) )
    return PLUGIN_HANDLED
...
public handle_buy_menu( p_userid, p_key )
{
  ...
  if ( ( u_Team == 1 && p_key == 4 ) || ( u_Team == 2 && p_key == 5 ) )
    handle_menu_weapon( p_userid, c7_vip_awp )
  if ( ( u_Team == 1 && p_key == 5 ) || ( u_Team == 2 && p_key == 4 ) )
    handle_menu_weapon( p_userid, c7_vip_autosniper )
...
{
  handle_buy_weapon( p_userid, c7_vip_awp )
}

public handle_buy_autosniper( p_userid )
{
  handle_buy_weapon( p_userid, c7_vip_autosniper )
}
...
public event_current_weapon( p_userid )
{
  ...
  if ( get_pcvar_num(c7_vip_awp) == 2 || get_pcvar_num(c7_vip_autosniper) == 2 )
    return PLUGIN_HANDLED

  switch ( get_user_weapon( p_userid, _, _ ) )
  {
    case CSW_AWP: handle_current_weapon( p_userid, c7_vip_awp, "weapon_awp", HUD_MSG_AWP_ALL, HUD_MSG_AWP_VIP )
    case CSW_SG550: handle_current_weapon( p_userid, c7_vip_autosniper, "weapon_sg550", HUD_MSG_AUTOSNIPER_ALL, HUD_MSG_AUTOSNIPER_VIP )
    case CSW_G3SG1: handle_current_weapon( p_userid, c7_vip_autosniper, "weapon_g3sg1", HUD_MSG_AUTOSNIPER_ALL, HUD_MSG_AUTOSNIPER_VIP )
  }
...
{
  if ( !is_user_connected(p_userid) || !get_pcvar_num(c7_vip_gunmenu_enable) )
    return PLUGIN_HANDLED
...
stock user_give_equip( p_userid )
{
  ...
  get_pcvar_string( c7_vip_gunmenu_grens, s_Grenades, charsmax(s_Grenades) )
...
{
  return g_CurrentRound >= get_pcvar_num(c7_vip_gunmenu_round)
}

stock print_gunmenu_roundsleft( p_userid=0 )
{
  new s_RoundString[7] = "rounds",
    n_RoundsLeft = get_pcvar_num(c7_vip_gunmenu_round) - g_CurrentRound
